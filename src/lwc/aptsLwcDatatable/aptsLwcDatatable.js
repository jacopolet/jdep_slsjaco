/**********************************************************************
Name: aptsLwcDatatable
Date: 06 July 2020
Description: Generic Datatable for LWC 
Author: Sai Sagar
...
export default class AptsLwcDatatable extends LightningElement {
    ...
    @api preSelected; //preSelectedOnDisplay
    @api disablePageSize=false;
    @track recordsToDisplay = []; //Records to be displayed on the page
    filteredRecords = []; //Filtered records available in the data table; valid type is Array
    @api selectedRecords = []; //OverallSelected records  in the data table; valid type is Array 
    ...
    connectedCallback() {
        ...
        console.log('page size disable =>'+this.disablePageSize);
    }

    handleTableAction(event){
        this.statuss = event.detail.row;
    ...
    setRecordsToDisplay(){
        ...
        this.selectedRecords.forEach((item) => {
            if(item.selected)
                this.preSelected.push(item[this.keyField]);
        })       
    ...
    handelRowsSelected(selectedRows) {
        ...
        if(this.recordsToDisplay && 
            ...
            this.recordsToDisplay.forEach((item)=>{                
                var row = new Object(); 
                row[this.keyField] = item[this.keyField];                
                if(selectedRows.includes(item[this.keyField]) && this.maxRowSelection != '1'){
                    row.selected = true;
                }else{
                    row.selected = false;
                }
        ...
        this.selectedRecords = this.mergeObjectArray(this.selectedRecords, this.pageSelectedRecords, `${this.keyField}`);       
        if(this.maxRowSelection === '1' && selectedRows && selectedRows.length > 0){
            this.totalSelected = 1;
            const filterSelected = this.selectedRecords.filter(({ Id }) => Id === selectedRows[0] );
    ...
    getSelectedRows(event) {
        ...
        for (let i = 0; i < selectedRows.length; i++){
            console.log(selectedRows[i][[this.keyField]]);
            selectedRecordIds.push(selectedRows[i][[this.keyField]]);
        }     
